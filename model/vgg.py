...
class Vgg19(torch.nn.Module):
    """
    Just those layers are taken into consideration which were used in the original paper
    'conv1_1', 'conv2_1', 'conv3_1', 'conv4_1', 'conv5_1' - used for style representation
    'conv4_2' - used for content representation (some experiments were made with conv2_2 and conv5_2)
    """
    def __init__(self, requires_grad=False, show_progress=False, use_relu=True):
        ...
        self.style_feature_maps_indices = list(range(len(self.layer_names)))  # all layers used for style representation except conv4_2
        self.style_feature_maps_indices.remove(4)  # conv4_2 (removed)
    ...
    def forward(self, x):
        ...
        model = vgg_outputs(layer1_1, layer2_1, layer3_1, layer4_1, conv4_2, layer5_1)
        return model
